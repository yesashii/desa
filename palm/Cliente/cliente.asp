...
<CENTER><BR>
<BR>Todos los clientes se crean automáticamente 
<BR>en condición de pago efectivo y $500.000 en crédito
<BR>
<BR>Los créditos sugeridos serán efectivos
...
sub dirDespacho3()
	...
	sql="select C.idcomuna, C.nombre " & _
		...
		"where c.idciudad=d.idciudad and replace(c.nombre,'Ñ','N') like '" & letra & "%' " & _
	...
	<FONT SIZE="2" face="verdana" COLOR="#000066"><B>Busqueda Alfabética</B><BR>
	<% 
...
<table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse">
	...
	<tr>
		...
		<td colspan="2" nowrap>&nbsp;<INPUT TYPE="button" value="Ver Resumen cliente" onClick="window.open('/palm/Clientes.asp?mirut=<%=RUT %>&empresa=<%=empresa %>')"></td>
	</tr>
...
%>
	...
	<form method="post" action="">
		...
		<tr>
			...
			<td nowrap>
			<%
			Sql="SELECT COUNT(*) FROM [SQLSERVER].[DesaERP].[dbo].[DIM_CtaCte] Where Codlegal='"& rut &"' and vb_aprobacion='P' "
			'response.Write("<br>" & sql)
			set rs=oConn.execute(Sql)
...
<%
	if rs.fields(0)<>0 THEN
%>
<FONT color='#ff0000'><b>Cliente con solicitud pendiente para la creación de nuevo local</FONT>
		<TH></TH>
		<BR>
		<BR>
<%
	...
	<TABLE bgcolor="#FFFFCC">
		...
		<TR>
			<TD align="center" >
				...
				<INPUT TYPE="hidden" name="empresa"    value="<%=request.querystring("empresa") %>">
...
':::::::::::::::
'Día o días Recepción | Horario inicio mañana | Horario final  mañana
...
select case TIPO
  case "NUEVO"
	...
	with response
	  ...
	  .write "<p align='center'>Petición para la creación de local creada con Éxito<br><br>"
  ...
  case "AGREGAR"
	...
	sqlexist= "SELECT COUNT(*) FROM [SQLSERVER].[DesaERP].[dbo].[DIM_CtaCte] Where Codlegal='"& rut &"' and vb_aprobacion='P'"
	set rsexist = oConn.execute(sqlexist)
	
	if 	rsexist.fields(0)=0 then 
		call ingnew()
		call enviocorreo()
	...
	with response
	 ...
	 ' chr(34)&"location='../../cliente/subeServer.asp?code="& CTACTE &"&paso=SendData&palm=1&empresa="& empresa &"'"& chr(34) &">")
	 if rsexist.fields(0)=0 then 
	  .write "<p align='center'>Generada solicitud para creación de cliente.<br><br>"
	  else
	  .write "<p align='center'>Cliente con solicitud pendiente para la creación de nuevo local, no se creo la nueva solicitud.<br><br>"
	  end if
	  .write "<input type='button' value='Salir'"
	  .write "onClick=""location='/palm/'""></p>"
	end with
...
if local<>1 then
	txtvb_aprob=",vb_aprobacion "
	vb_aprobacion=",'P'"
end if

SQLctacte = "INSERT INTO [SQLSERVER].[DesaERP].[dbo].[DIM_CtaCte] (EMPRESA, TipoCtaCte, CtaCte, Codlegal, RazonSocial, "&_
...
"Moneda, Vigencia, VigenciaCredito, Estado, Comentario1, ANALISISCTACTE12, ANALISISCTACTE25,ANALISISCTACTE10"& txtvb_aprob &") "&_
" VALUES "&_
...
"'0', '0', '0', '0', 'PS', '"& VIGENCIA &"', '"& VCREDITO &"', '"& ESTADO &"','" & OBS & "','02','N','" & ASEG & "'"& vb_aprobacion &")"  

if local=1 then
	SQLctacte=replace(SQLctacte,"[SQLSERVER].[DesaERP].[dbo].[DIM_CtaCte]","Flexline.CtaCte")
end if
...
SQLctacteD1 = "INSERT INTO [SQLSERVER].[DesaERP].[dbo].[DIM_CtaCteDirecciones] "&_
...
if local=1 then
	SQLctacteD1=replace(SQLctacteD1,"[SQLSERVER].[DesaERP].[dbo].[DIM_CtaCteDirecciones]","CtaCteDirecciones")
end if
...
SQLctacteD2 = "INSERT INTO [SQLSERVER].[DesaERP].[dbo].[DIM_CtaCteDirecciones] "&_
...
if local=1 then
	SQLctacteD2=replace(SQLctacteD2,"[SQLSERVER].[DesaERP].[dbo].[DIM_CtaCteDirecciones]","CtaCteDirecciones")
end if
