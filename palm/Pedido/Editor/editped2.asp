...
if request.QueryString("step") = "camclie" then call cambiar_cliente()
Sub Lista_Notas()

fmd = right(year(date), 2) & right("00" & month(date), 2) & right("00" & day(date), 2)

SQL0 =  "SELECT Flexline.FX_PEDIDO_PDA.nota, Flexline.FX_PEDIDO_PDA.Cliente, "&_
		"Flexline.CtaCte.RazonSocial, Flexline.FX_PEDIDO_PDA.oc, "&_
		"Flexline.FX_PEDIDO_PDA.fechaentrega "&_
		"FROM Flexline.FX_PEDIDO_PDA LEFT OUTER JOIN "&_
		"Flexline.CtaCte ON Flexline.FX_PEDIDO_PDA.Cliente = Flexline.CtaCte.CtaCte "&_
		"WHERE (Flexline.FX_PEDIDO_PDA.vendedor = '"& Vnd &"') AND "&_
		"(Flexline.FX_PEDIDO_PDA.fecha = '"& fmd &"') AND "&_
		"(Flexline.FX_PEDIDO_PDA.estado = 'np') AND (Flexline.CtaCte.Empresa = 'DESA') "&_
...
%>
 <tr>
	...
	<td><a href="?nuser=<%= Nuser %>&step=edit&nota=<%= rs1.fields("Nota") %>"><img src="edit.jpg" alt="Editar" width="10" height="10" border="0" /></a>&nbsp;</td>
	<td align="center"><%= rs1.fields("Nota") %></td>
	<td><%= left(rs1.fields("razonsocial"), 15) %></td>
 </tr>
<%
rs1.movenext
loop
...
SQL="SELECT * FROM Flexline.FX_PEDIDO_PDA "&_
...
%>    <% if rs.fields("estado")="OK" then modd = "disabled" %> 
  ...
  <tr>
	...
	onclick="document.location='?step=camclie&nuser=<%= Nuser %>&nt=<%= nota %>'"></td>
  </tr>
  <tr>
...
%>  <tr>
  ...
  <tr>
    <td colspan="2" nowrap><%= rs.fields("producto"& right("00"& x,2)) %> - 
      <%
Sql2="SELECT GLOSA FROM FX_PRODUCTO "&_
"WHERE     (PRODUCTO = '"& rs.fields("producto"& right("00"& x,2)) &"')"
...
Sub Actualizar()
'request.form("nota")
SQLG="SELECT * FROM Flexline.FX_PEDIDO_PDA "&_
...
SQL = "UPDATE Flexline.FX_PEDIDO_PDA "&_
...
End Sub 'Eliminar
sub cambiar_cliente()
SQL="SELECT  * FROM  (SELECT  Flexline.FX_vende_man.NU_2 AS Nuser, "&_
    "Flexline.FX_PEDIDO_PDA.vendedor, Flexline.FX_PEDIDO_PDA.Cliente, "&_
    "Flexline.FX_PEDIDO_PDA.nota FROM Flexline.FX_vende_man INNER JOIN "&_
    "Flexline.FX_PEDIDO_PDA ON Flexline.FX_vende_man.nombre = Flexline.FX_PEDIDO_PDA.vendedor "&_
	"WHERE (NU_2 = '"& replace("000"&request.QueryString("Nuser")) &"') AND (nota = '"& request.QueryString("nt") &"')) Pedido"
set rs= oConn.execute(SQL)
%>
...
<%= replace(replace(request.querystring,"&","<br>")," ", "&nbsp;")%>
<%
end sub ' cambiar_cliente()
%>
</center>
</body>
</html>